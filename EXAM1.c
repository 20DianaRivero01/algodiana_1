...
int main(int argc, char ** argv) {
    ...
    for (i = 1; i <= finish; i++) { // as long as my is less than the number you entered
        divisores = 0; //Divisors have 0 on their counter, this is to identify if there are numbers that were divided by the primes. 
        if (finish % i == 0) {//First of all we have to identify if the number is a divisor to our value
            ...
            if (divisores == 1) { //in case it has been divisible, it enters this loop and it is verified that there is a cousin
                printf("%d ", i); // prints
